<template>
    <div ref="container" class="overview-container">
        ...
        <div style="height: 90%;"><svg ref="chart"></svg></div>
...
export default {
    ...
    methods: {
        initChart() {
            ...
            // 分区比例
            const verticalWidth = width * 0.1;
            const horizontalWidth = width * 0.65;
            const rectWidth = width * 0.1

            const heightZhexian = height * 0.2;

            // 绘制垂直堆叠柱状图
            const verticalChart = this.createVerticalCircleChart(svg, verticalWidth, height);
            ...
            // console.log((horizontalChart.horizontalPoints[0].bottom.y + horizontalChart.horizontalPoints[0].top.y) / 2, pieRes[0]);
            // for (let i = 0; i < this.verticalData[0].values.length; i++) {
            //     // this.drawIner(svg, horizontalChart.horizontalPoints[i].bottom, verticalChart.points[i].bottom, verticalChart.points[i].top, horizontalChart.horizontalPoints[i].top, this.colors[i])
            ...
            // }
            // this.createHorizontalChart(svg, horizontalWidth, verticalWidth, height);
 ...
 */
        ...
        createVerticalChart(svg, width, height) {
            ...
            layerData.forEach((layer, layerIndex) => {
                const bars = chartGroup.selectAll(`.layer-${layerIndex}`)
                    .data(layer)
                    .enter().append('rect')
                ...
                bars
                    ...
                    .attr('height', d => yScale(baseHeight) - yScale(baseHeight + d.value));
        ...
        createVerticalCircleChart(svg, width, height) {
            ...
            layerData.forEach((layer, layerIndex) => {
                const circles = chartGroup.selectAll(`.layer-${layerIndex}`)
                    .data(layer)
                    .enter().append('circle')
                    .attr('class', `layer-${layerIndex}`)
                ...
                circles
                    ...
                    .attr('cy', d => yScale(this.horizontalData[layerIndex].label) + 0.5 * yScale.bandwidth())
                    .attr('r', 0.3 * yScale.bandwidth());
                // .attr('r', d => Math.sqrt(d.value / Math.PI));

                // 记录右侧端点
                rightPoints.push(...layer.map(d => ({
                    top: {
                        x: xScale(d.label) + xScale.bandwidth() / 2 + Math.sqrt(d.value / Math.PI),
                        y: yScale(baseHeight + d.value / 2 + LAYER_GAP) - Math.sqrt(d.value / Math.PI)
                    },
                    bottom: {
                        x: xScale(d.label) + xScale.bandwidth() / 2 + Math.sqrt(d.value / Math.PI),
                        y: yScale(baseHeight + d.value / 2 + LAYER_GAP) + Math.sqrt(d.value / Math.PI)
        ...
        createHorizontalChart(svg, width, verticalWidth, height, offsetX) {
            ...
            chartGroup.selectAll('.horizontal-bar')
                .data(stackedData)
                .enter().append('g')
                .attr('fill', (d, i) => this.colorsHor[i])
                .selectAll('rect')
                .data(d => d)
                .enter().append('rect')
                .attr('x', d => xScale(d[0]))
            ...
            this.pieData.forEach((d, i) => {
                ...
                pieGroup.selectAll('.pie-slice')
                    ...
                    .attr('stroke', (d) => {
                        ...
                        return centralAngle > 180 ? 'red' : this.colorsHor[this.colorsHor.length - 1];
                    })
                    .attr('stroke-width', 2)
                    .attr('fill', "none");
            });
